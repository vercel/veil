...
// File, FS, RenderHookContext, RenderHook, and — when the graph
// indicates dependency relationships — the consumer-side `Dependency`
// union and/or per-consumer DependentHook types.
func VeilTypes(k *config.Kind, variables map[string]*veilv1.Variable, graph *KindGraph, sharedImport string) (string, error) {
	...
	// When a shared module is configured, import the kind-independent
	// symbols from it instead of inlining them. The per-kind types below
	// reference exactly these. Unset → inline path, byte-identical to the
	// original output.
	if sharedImport != "" {
		fmt.Fprintf(&b, "import type { File, Std, Os, Fetch, Resource, RegistryVariables, ValidationResult } from '%s';\n\n", sharedImport)
	}
...
// SharedVeilTypes renders the shared veil-types module: the kind-independent
// declarations VeilTypes imports (rather than inlines) when a sharedImport is
// given — the host APIs (Std/Os/Fetch/YamlCodec/Stats), File, Override/
// Metadata/Resource, ValidationIssue/ValidationResult, and RegistryVariables
// (identical across every kind in a project). `veil build` writes it once per
// project when generators.shared_types is set.
func SharedVeilTypes(variables map[string]*veilv1.Variable) (string, error) {
	varsIface, err := variablesInterface(variables)
	if err != nil {
		return "", err
	}
	...
	var b strings.Builder
	b.WriteString("// Generated by veil build — do not edit.\n")
	b.WriteString("// Shared, kind-independent veil-types imported by every kind's\n")
	b.WriteString("// veil-types.ts (see generators.shared_types).\n\n")
	b.WriteString(hostInterfaces)
	b.WriteString("\n")
	b.WriteString(fileInterface)
	b.WriteString("\n")
...
// renderHookContextTemplate is the per-kind RenderHookContext interface
// (%s = the kind's Spec type). It references Resource/RegistryVariables/
// Std/Os/Fetch — emitted inline (resourceTypes/hostInterfaces) or imported
// from the shared module depending on VeilTypes' sharedImport.
...
// validateHookInterface is the per-kind ValidateHook (references the local
// RenderHookContext + FS and the shared ValidationResult).
