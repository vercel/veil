...
func dependentInterfaces(n *KindNode, packageMode bool) (string, error) {
	...
	for _, edge := range dependents {
		...
		fmt.Fprintf(&b, "  /** This kind's own files — the same FS its own hooks see,\n")
		fmt.Fprintf(&b, "   *  typed accessors included. Read from it to hand the consumer\n")
		fmt.Fprintf(&b, "   *  something this kind ships: `fs.add(path, ctx.selfFS.get(...)\n")
		fmt.Fprintf(&b, "   *  .getContent())`. Nothing is read back, so changes here do not\n")
		fmt.Fprintf(&b, "   *  affect this resource's own render. */\n")
		b.WriteString("  selfFS: FS;\n")
		fmt.Fprintf(&b, "  /** The consumer resource that declared a dependency on us. */\n")
		fmt.Fprintf(&b, "  consumer: Resource<%s>;\n", consumerSpecName)
		fmt.Fprintf(&b, "  /** Path of the consumer resource file being rendered, relative\n")
		fmt.Fprintf(&b, "   *  to the veil project root. */\n")
