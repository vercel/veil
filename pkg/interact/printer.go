...
// NewPrinter returns a printer that always logs via slog. When pretty is
// true it *also* writes styled output to w; when false the printer just
// logs (the message flows through the logger to wherever --log-paths
// points, the rolling file by default). Callers pass pretty=false for
// JSON output and under --quiet.
func NewPrinter(w io.Writer, pretty bool) Printer {
	return &printer{w: w, theme: NewTheme(), pretty: pretty}
}
...
// that writes to os.Stderr if SetDefault hasn't been called — styled
// unless the output format is JSON.
func Default() Printer {
	if p := defaultPrinter.Load(); p != nil {
		return *p
	}
	return NewPrinter(os.Stderr, !IsJSON())
}
