...
func (s *BuildSuite) TestBuildEmbedsSourceSchemas() {
	...
	s.Require().NoError(os.WriteFile(filepath.Join(kindDir, "hooks", "src", "bump.ts"), []byte(`import type { FS, RenderHook, RenderHookContext } from './veil-types';

const bump: RenderHook = {
  render(ctx: RenderHookContext, fs: FS): FS {
    const file = fs.getSourcesDeploymentJson();
...
`), 0644))
	...
	renderArr = append(renderArr, map[string]any{"path": "./hooks/src/bump.ts"})
