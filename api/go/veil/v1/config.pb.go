...
type KindDefinition struct {
	...
	//
	// Entries are typed as `google.protobuf.Value` so each one can be
	// EITHER a bare string path (no declared schema) OR an object
	// matching SourceDefinition (path plus optional `schema`) — the
	// same polymorphism `hooks` already uses, for the same reason:
	// protojson can't express "string OR struct" on a typed field. The
	// build pipeline converts each Value into a SourceDefinition before
	// compilation.
...
// SourceDefinition is a single source entry in kind.json: a path to
// the source configuration file plus optional `schema` — a JSON
// Schema file describing the source's shape, resolved the same way
// KindDefinition.schema resolves the spec schema. A source with no
// `schema` behaves exactly as before: raw content, no per-source
// typing. Declaring `schema` makes the source's final content a
// contract the runner enforces regardless of whether any hook binds
// to it as typed (see RenderHookDefinition.source).
type SourceDefinition struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// Path to the source file. Resolved relative to the enclosing
	// kind.json file.
	Path string `protobuf:"bytes,1,opt,name=path,proto3" json:"path,omitempty"`
	// Path to a JSON Schema file describing this source's shape.
	// Resolved relative to the enclosing kind.json file. Optional —
	// omit for a source with no declared shape.
...
// `access`/`source` fields). The wire format is whatever the user
...
type RenderHookDefinition struct {
	...
	// Path referencing this kind's declared `sources` entry this hook
	// is bound to. When set, the hook receives (and must return) the
	// parsed object for that one source instead of the whole-bundle
	// FS: `render(ctx, obj)` in place of `render(ctx, fs)`. Must match
	// a `SourceDefinition.path` in the same kind.json's `sources` list.
	// Empty (the default) preserves today's whole-bundle contract.
...
func (x *RenderHookDefinition) GetSource() string {
	if x != nil {
		return x.Source
	}
	return ""
...
const file_veil_v1_config_proto_rawDesc = "" +
	...
	"\x06access\x18\x02 \x01(\v2\x13.veil.v1.HookAccessR\x06access\x12\x16\n" +
...
var file_veil_v1_config_proto_msgTypes = make([]protoimpl.MessageInfo, 18)
var file_veil_v1_config_proto_goTypes = []any{
	...
	(*HooksDefinition)(nil),      // 11: veil.v1.HooksDefinition
	(*RenderHookDefinition)(nil), // 12: veil.v1.RenderHookDefinition
	(*HookAccess)(nil),           // 13: veil.v1.HookAccess
	(*EnvAccess)(nil),            // 14: veil.v1.EnvAccess
	(*DependentDefinition)(nil),  // 15: veil.v1.DependentDefinition
	nil,                          // 16: veil.v1.VeilConfigDefinition.VariablesEntry
	nil,                          // 17: veil.v1.VeilConfigDefinition.RegistriesEntry
	nil,                          // 18: veil.v1.KindDefinition.VariablesEntry
	(*structpb.Value)(nil),       // 19: google.protobuf.Value
}
var file_veil_v1_config_proto_depIdxs = []int32{
	19, // 0: veil.v1.VeilConfigDefinition.kinds:type_name -> google.protobuf.Value
	16, // 1: veil.v1.VeilConfigDefinition.variables:type_name -> veil.v1.VeilConfigDefinition.VariablesEntry
	17, // 2: veil.v1.VeilConfigDefinition.registries:type_name -> veil.v1.VeilConfigDefinition.RegistriesEntry
	...
	19, // 8: veil.v1.Variable.default:type_name -> google.protobuf.Value
	19, // 9: veil.v1.Variable.enum:type_name -> google.protobuf.Value
	19, // 10: veil.v1.KindDefinition.sources:type_name -> google.protobuf.Value
	11, // 11: veil.v1.KindDefinition.hooks:type_name -> veil.v1.HooksDefinition
	18, // 12: veil.v1.KindDefinition.variables:type_name -> veil.v1.KindDefinition.VariablesEntry
	19, // 13: veil.v1.HooksDefinition.render:type_name -> google.protobuf.Value
	15, // 14: veil.v1.HooksDefinition.dependents:type_name -> veil.v1.DependentDefinition
	19, // 15: veil.v1.HooksDefinition.validate:type_name -> google.protobuf.Value
