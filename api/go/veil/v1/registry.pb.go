...
type Kind struct {
	...
	// Raw JSON Schema text for every source that declared `schema` in
	// its KindDefinition.SourceDefinition, keyed the same way `sources`
	// is (path relative to the kind directory). Inlined here — like
	// DependentHook.params_schema — so render can validate a source's
	// content without filesystem access to the original kind
	// directory. Sources with no declared schema have no entry here.
...
type Hook struct {
	...
	// Path into the compiled Kind.sources/source_schemas maps this hook
	// is bound to (copied from KindDefinition.RenderHookDefinition.source
	// at compile time). Empty (the default) means this is a
	// whole-bundle hook: RenderHook(ctx, fs). Non-empty means the
	// runner invokes RenderHookTyped(ctx, obj) instead, parsing/
	// re-serializing that one source around the call.
...
const file_veil_v1_registry_proto_rawDesc = "" +
	...
	"\xbaH\a\xc8\x01\x01r\x02\x10\x01R\x06schema\"\x91\x01\n" +
	"\x04Hook\x12\x1e\n" +
	"\x04name\x18\x01 \x01(\tB\n" +
	"\xbaH\a\xc8\x01\x01r\x02\x10\x01R\x04name\x12$\n" +
	"\acontent\x18\x02 \x01(\tB\n" +
	"\xbaH\a\xc8\x01\x01r\x02\x10\x01R\acontent\x12+\n" +
	"\x06access\x18\x03 \x01(\v2\x13.veil.v1.HookAccessR\x06access\x12\x16\n" +
	"\x06source\x18\x04 \x01(\tR\x06source\"\xa3\x01\n" +
	"\rDependentHook\x122\n" +
